...
if [[ $# -gt 0 ]]; then
    err "newTab: takes no arguments (got '$*')"
    exit 2
fi
...
open_in_new_tab() {
    ...
    if [[ $status -ne 0 ]]; then
        ...
        if [[ "$err_output" == *"not allowed to send keystrokes"* ]]; then
            err ""
            err "newTab: needs macOS Accessibility permission to send Cmd+T."
...
if [[ -n "$term" && "$term" != "Apple_Terminal" ]]; then
    err "newTab: TERM_PROGRAM=$term not yet supported, falling back to Apple Terminal"
fi
...
if [[ "${front_count:-0}" -eq 0 ]]; then
    err "newTab: no front Terminal window — opening a new window instead"
