---
name: newClaudeTab
description: Open a new tab in the current macOS Terminal window and start a fresh `claude` session in the same working directory. Use whenever the user invokes /newClaudeTab, or asks to "open a new terminal tab with claude", "start another claude session in a new tab", "spin up a fresh claude alongside this one", "open another claude in this window". Differs from /cloneClaudeConversation — /cloneClaudeConversation forks the *current* conversation (resumes its transcript), whereas /newClaudeTab starts a completely fresh session with no prior context. The current session keeps running unchanged.
---
...
Use this when you want a second Claude session in parallel **without** carrying over the current conversation. If you instead want to fork the current transcript so both threads share history up to this point, use `/cloneClaudeConversation` instead.
...
```bash
~/.claude/skills/newClaudeTab/scripts/newClaudeTab.sh
```
...
This is the same setup used by `/cloneClaudeConversation here`. If you've already enabled it for `/cloneClaudeConversation`, `/newClaudeTab` works without further action.
